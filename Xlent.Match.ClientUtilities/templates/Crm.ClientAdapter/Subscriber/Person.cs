...
{
    public class PersonsSubscriber : BaseClass
    {
        /// <summary>
        ...
        {
            await Task.Run(() => BaseClass.HandleRequests("Person", HandleOne));
        }
        ...
        private static SuccessResponse HandleOne(Request request)
        {
            ...
            {
                case "Get":
                    if (person == null) throw new NotFoundException(request.ClientName, request.EntityName, request.KeyValue);
                ...
                case "Update":
                    ...
                    break;
                case "Create":
                    if (person != null) throw new MovedException(person.Model.Id.ToString());
